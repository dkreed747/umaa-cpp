...
static std::shared_ptr<arlcore::LogReportService> logReportSvc;
...
static void defaultFactory(const log4cxx::LoggerPtr& rootLogger) {
  log4cxx::PatternLayoutPtr patternPtr = new log4cxx::PatternLayout("[%p] %d{HH:mm:ss.SSSS} %M - %m%n");
  log4cxx::ConsoleAppenderPtr consoleAppenderPtr = new log4cxx::ConsoleAppender(patternPtr);
