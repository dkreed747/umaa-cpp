...
- `INVALID_SET` is retryable (elements or disposals may still be in flight for the metadata), but `STALE_METADATA` is not: the provided metadata is older than the latest metadata already received for that set and will never become valid. Consumers that re-derive from retained metadata every poll must stop retrying on `STALE_METADATA` and wait for fresh metadata.