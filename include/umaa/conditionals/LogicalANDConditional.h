...
#ifndef INCLUDE_UMAA_CONDITIONALS_LOGICALANDCONDITIONAL_H_
#define INCLUDE_UMAA_CONDITIONALS_LOGICALANDCONDITIONAL_H_
...
//! @brief This class defines a logical AND operator for a set of conditionals. The conditional is true when both
//! conditionals referenced by conditionalID1 and conditionalID2 evaluate to true.
class LogicalANDConditional : public ConditionalBase {
 public:
  //! \brief Constructor
  //! \param conditional The base UMAA conditional type
  //! \param derivedConditional The specialized LogicalANDConditional UMAA type
  LogicalANDConditional(
    const ConditionalType& conditional,
    const LogicalANDConditionalType& derivedConditional) :
    ConditionalBase(conditional), derivedConditional_(derivedConditional) {
    if (!isValidSpecialization(conditional, derivedConditional, LogicalANDConditionalTypeTopic)) {
  ...
  //! be evaluated (i.e. the conditionals whose evaluations are ANDed together)
  ...
  LogicalANDConditionalType getSpecializedConditional() const {
    return derivedConditional_;
  }

 private:
  LogicalANDConditionalType derivedConditional_;
  std::weak_ptr<ConditionalBase> firstConditional_;
  std::weak_ptr<ConditionalBase> secondConditional_;
};

}  // namespace arlcore::umaa::conditional
#endif  // INCLUDE_UMAA_CONDITIONALS_LOGICALANDCONDITIONAL_H_
